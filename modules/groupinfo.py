...
def draw_stylized_avatar(image, avatar_image, position, size, border_color=(220, 220, 220), border_thickness=15,
    ...
    icon_font = ImageFont.truetype("modules/cache/font/NotoEmoji-Bold.ttf", icon_size)
...
def calculate_text_height(content, font, image_width):
    lines = split_text_into_lines(content, font, ImageFont.truetype("modules/cache/font/NotoEmoji-Bold.ttf", font.size),
                                  int(image_width * 0.6))
    th = font.getbbox("Ay")[3] - font.getbbox("Ay")[1]
    lh = int(th * 1.2)
    return len(lines) * lh
...
def get_group_info_text(group, get_name):
    msg = f"📋 Tên nhóm: {truncate_text(group.name)}\n"
    msg += f"👤 Trưởng nhóm: {truncate_text(get_name(group.creatorId))}\n"
    admin_ids = group.adminIds
    num_admins = len(admin_ids)
    msg += f"👤 Số phó nhóm: {num_admins}\n"
    group_type = "Cộng Đồng" if group.type == 2 else "Nhóm"
    msg += f"💼 Phân loại: {group_type}\n"
    if group.updateMems:
    ...
    msg += f"📆 Thời gian tạo: {formatted_time}\n"
    return msg
