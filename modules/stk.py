        ...
        if message.startswith('.stk '):
            ...
            if url.endswith('.webp'):
                if is_valid_media_url(url):
                    ...
                    client.sendMessage(
                        Message(text="URL không hợp lệ hoặc không phải là ảnh."),
                        thread_id=thread_id,
                        thread_type=thread_type
                    )
            else:
                client.sendMessage(
                    Message(text="Vui lòng cung cấp URL có định dạng .webp."),
                    thread_id=thread_id,
                    thread_type=thread_type
                )
        else:
            client.sendMessage(
                Message(text="Vui lòng sử dụng lệnh 'stk' để tạo sticker."),
                thread_id=thread_id,
                thread_type=thread_type
            )
...
def send_sticker(client, staticImgUrl, animationImgUrl, thread_id, thread_type):
    try:
        ...
        client.sendMessage(
            Message(text=f"Sticker đã được tạo thành công! Bạn có thể xem tại: {animationImgUrl}"),
            thread_id=thread_id,
            thread_type=thread_type
        )
