...
des = {'version': "1.3.0", 'credits': "Vũ Xuân Kiên", 'description': "weo căm", 'power': "Thành viên"}
...
def draw_stylized_avatar(image, avatar_image, position, size, border_color=(220, 220, 220), border_thickness=15,
    ...
    icon_font = ImageFont.truetype("modules/cache/font/Segoe UI Emoji.ttf", icon_size)
...
def calculate_text_height(content, font, image_width):
    lines = split_text_into_lines(content, font, ImageFont.truetype("modules/cache/font/Segoe UI Emoji.ttf", font.size),
                                  int(image_width * 0.6))
    th = font.getbbox("Ay")[3] - font.getbbox("Ay")[1]
    lh = int(th * 1.2)
    return len(lines) * lh
...
def process_image_with_info(avatar_url, cover_url, author_name, user_info_text, is_active_pc, is_active_web):
    ...
    try:
        if cover_url == "https://cover-talk.zadn.vn/default" and os.path.exists("modules/cache/vxkiue.jpg"):
            ci = Image.open("modules/cache/vxkiue.jpg").convert("RGB")
    ...
    draw = ImageDraw.Draw(image)
    f = get_font_size(63)
    ef = ImageFont.truetype("modules/cache/font/Segoe UI Emoji.ttf", f.size)
    text_x = separator_x
    text_y = ih // 2
...
def get_user_info_text(user):
    dob = user.dob or user.sdob or "Ẩn"
    ...
    business = "Có" if business else "Không có"
    ...
    gender_string = {
        0: 'Nam',
        1: 'Nữ',
    }.get(user.gender, 'Không xác định') if hasattr(user, 'gender') else 'Không xác định'

    user_info_text = (
        f"🆔 Username: {truncate_text(user.username) if hasattr(user, 'username') else 'Không có'}\n"
        f"👤 Tên: {truncate_text(user.zaloName) if hasattr(user, 'zaloName') else 'Không có'}\n"
        f"🚻 Giới tính: {truncate_text(gender_string)}\n"
        f"💼 Tài khoản Business: {truncate_text(business)}\n"
        f"🎂 Ngày sinh: {truncate_text(dob)}\n"
        f"📆 Ngày tạo tài khoản: {truncate_text(createTime)}\n"
        f"📜 Lần cuối hoạt động: {truncate_text(lastAction)}\n"
    )
